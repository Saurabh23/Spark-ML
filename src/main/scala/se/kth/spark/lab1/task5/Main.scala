...
object Main {
  def main(args: Array[String]) {
    ...
    val paramGrid = new ParamGridBuilder().addGrid(myLR.maxIter,Array(5,10,30,50,100,200,300)).addGrid(myLR.regParam,Array(.01,.05,.1,.5,.7,.9,1)).build()
    ...
    //print rmse of our modelx``x`
    println(s"Best model by Cross Validation. HyperParams. Max Iterations: ${lrModel.getMaxIter}  Regularization Parameters: ${lrModel.getRegParam}")
    println(s"Best model by Cross Validation. RMSE : ${lrModel.summary.rootMeanSquaredError}")
